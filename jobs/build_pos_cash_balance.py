...
def main() -> None:
    ...
    logger = setup_logger("build_pos_cash_balance", paths.logs)
    spark = build_spark_session(config)

    pos_cash = read_csv_table(spark, paths.raw / config.raw["tables"]["pos_cash_balance"])
    features, _ = build_pos_cash_features(pos_cash)

    result = assert_unique_keys(features, ["SK_ID_CURR"], "pos_cash_features")
    if not result.passed:
        raise ValueError(result.details)

    output_path = paths.interim / "pos_cash_features"
    write_parquet(features, output_path)
    logger.info("Wrote POS/CASH features to %s", output_path)
